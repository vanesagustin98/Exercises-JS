function getOptimalPath(path) {
    let numeroNiveles = path.length
    let piramide = path.map(row => Array(row.length).fill(0));
    piramide[numeroNiveles-1] = path[numeroNiveles-1]
    for (let nivel = numeroNiveles-2; nivel >= 0; nivel--) {
        for (let i = 0; i < piramide[nivel].length; i++) {
            let min = Math.min(piramide[nivel+1][i],piramide[nivel+1][i+1])
            piramide[nivel][i] = path[nivel][i] + min
        }
    }
    return piramide[0][0]
}
