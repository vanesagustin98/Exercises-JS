function printTable(gifts) {
    ...
    for (let i = 0; i < gifts.length; i++) {
        maxGift = Math.max(
            maxGift, gifts[i].name.length >= 4 ? 
            gifts[i].name.length : 4);
        maxQuantity = Math.max(
            maxQuantity, gifts[i].quantity.toString().length >= 8 ?
            gifts[i].quantity.toString().length : 8);
    }

    let max = maxGift + maxQuantity + 7;

    let table = '+'.repeat(max) + '\n' + '| Gift' + ' '
                    .repeat(maxGift - 3) + '| Quantity' + ' '
                    .repeat(maxQuantity - 8) + ' |\n' + '| ' + '-'
                    .repeat(maxGift) + ' | ' + '-'
                    .repeat(maxQuantity) + ' |';

    for (let i = 0; i < gifts.length; i++) {
        table += '\n' + '| ' + gifts[i].name + ' '
        .repeat(maxGift - gifts[i].name.length) 
        + ' | ' + gifts[i].quantity + ' '
        .repeat(maxQuantity - gifts[i].quantity.toString().length) + ' |';
    }
